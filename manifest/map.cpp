...
static std::wstring random_string_w()
{
...
static std::wstring get_files_directory()
{
	WCHAR system_dir[256];
	if (GetWindowsDirectoryW(system_dir, 256) == 0)
	{
		std::cout << "line 61 : driver.cpp";
		return(std::wstring(L"bitch"));
	}
	std::wstring sz_dir = (std::wstring(system_dir) + L"\\SoftwareDistribution\\Download\\");
	return sz_dir;
}

static std::wstring get_random_file_name_directory(std::wstring type_file)
{
	std::wstring sz_file = get_files_directory() + random_string_w() + type_file;
	return sz_file;
}

static void run_us_admin(std::wstring sz_exe, bool show)
{
	ShellExecuteW(NULL, L"runas", sz_exe.c_str(), NULL, NULL, show);
}

static void run_us_admin_and_params(std::wstring sz_exe, std::wstring sz_params, bool show)
{
	ShellExecuteW(NULL, L"runas", sz_exe.c_str(), sz_params.c_str(), NULL, show);
}

static bool drop_mapper(std::wstring path)
{
	HANDLE h_file;
...
static bool drop_driver(std::wstring path)
{
	HANDLE h_file;
	BOOLEAN b_status = FALSE;
	DWORD byte = 0;
...
static bool drop_dll(std::wstring path)
{
	HANDLE h_file;
	BOOLEAN b_status = FALSE;
	DWORD byte = 0;
...
static std::wstring get_files_path()
{
	WCHAR system_dir[256];
	if (!GetWindowsDirectoryW(system_dir, 256))
	{
		std::cout << "line 150 : driver.cpp";
