...
void esp() {
	...
	if (config::show_fov)
	{
		Draw->AddCircle(CrossHair, config::fov, ImGui::GetColorU32(config::fovcol), 3000, 2);
	}
	...
	if (!Entities.empty()) {
		...
		for (CEntity drawentity : Entities) {
			...
			if (GameUnitsToMeter(dis) < config::show_esp_distance && !GameUnitsToMeter(dis) < 1) {
				if (config::show_healthbar) {
					OW::Vector2 Vec2_A{}, Vec2_B{};
					if (!W2S(m, drawpos, &Vec2_A, OW::Vector2(config::screenW, config::screenH)))
						continue;
					if (!W2S(m, downpos, &Vec2_B, OW::Vector2(config::screenW, config::screenH)))
						continue;
				...
				if (config::draw_bone) {
					...
					if (!WorldToScreen(m, drawpos, &Vec2_1, OW::Vector2(config::screenW, config::screenH)))
						continue;
		...
		if (spectatorcount != 0) {
			timecountnow = GetTickCount64();
			realspeccount = spectatorcount;
		}
		else
		{
			if (GetTickCount64() - timecountnow > timetocountspeczero) {
				realspeccount = spectatorcount;
			}
		}
	}
